...
@objc public class VideoPlayerView: NSObject, FlutterPlatformView, FlutterStreamHandler {
    ...
    // Native frame-liveness monitor. This is intentionally separate from the
    // periodic playback-position observer: advancing time does not prove that
    // AVPlayerLayer / FlutterTexture is presenting fresh video frames.
    var frameMonitorOutput: AVPlayerItemVideoOutput?
    var frameMonitorItem: AVPlayerItem?
    var frameMonitorTimer: Timer?
    var firstFrameSent = false
    var lastFrameHeartbeatAt: TimeInterval = 0
    ...
    public init(
        ...
        if useTextureView {
            ...
            light.onFirstFramePresented = { [weak self] in
                self?.sendFirstFrameIfNeeded()
            }
            light.playerLayer.player = player
            light.playerLayer.videoGravity = .resizeAspectFill
            lightView = light
        } else {
    ...
    func rebindInlinePlayer() {
        if usesTextureView {
            ...
            lightView.playerLayer.videoGravity = .resizeAspectFill
        } else {
    ...
    func prepareAudioSession() {
        do {
            ...
            // Without this, UIImpactFeedbackGenerator is often silent while
            // the movie-playback session is active (player follow/like haptics).
    ...
    public func handleMethodCall(call: FlutterMethodCall, result: @escaping FlutterResult) {
        ...
        case "setUserInteractionEnabled":
            ...
            lightView?.isUserInteractionEnabled = enabled
            if usesViewControllerDisplay {
                playerViewController.view.isUserInteractionEnabled = enabled
            }
            result(nil)
        case "captureCurrentFrame":
            handleCaptureCurrentFrame(call: call, result: result)
