...
void main() {
  ...
  test(
    'diskCacheBytes returns 0 off Android and forwards on Android',
    () async {
      debugDefaultTargetPlatformOverride = TargetPlatform.iOS;
      expect(await NativeVideoPlayerCache.diskCacheBytes(), 0);
      expect(methodCalls, isEmpty);

      debugDefaultTargetPlatformOverride = TargetPlatform.android;
      messenger.setMockMethodCallHandler(methodChannel, (call) async {
        methodCalls.add(call);
        if (call.method == 'getDiskCacheBytes') return 12345;
        return null;
      });
      expect(await NativeVideoPlayerCache.diskCacheBytes(), 12345);
      expect(
        methodCalls,
        contains(
  ...
  test(
    'clearDiskCache is a no-op off Android and calls through on Android',
    () async {
      debugDefaultTargetPlatformOverride = TargetPlatform.iOS;
      ...
      expect(
        ...
        contains(
          isA<MethodCall>()
              .having((c) => c.method, 'method', 'clearDiskCache')
              .having(
                (c) => c.arguments,
                'arguments',
                containsPair('cacheMaxBytes', 100 * 1024 * 1024),
              ),
        ),
      );
    },
