...
class VideoPlayerMethodHandler(
    ...
    private fun handleLoad(call: MethodCall, result: MethodChannel.Result) {
        ...
        // Reply to Flutter as soon as prepare() is issued. Waiting for
        // STATE_READY before result.success() races with ExoPlayer: READY can
        // fire before the listener is attached (or be swallowed when a second
        // load supersedes the first), leaving the Dart loadUrl Future hung
        // until the 15s timeout — seen as loading→paused then a wasted retry.
        // Playability is signaled via the "loaded" activity event instead.
        var loadedEventSent = false
        fun sendLoadedIfNeeded() {
            if (loadedEventSent) return
        ...
        try {
            ...
            result.error("LOAD_ERROR", e.message ?: "prepare failed", null)
            return
        }

        // Unblock the Flutter MethodChannel immediately.
        result.success(null)

        // Cover sync-ready: prepare left the player READY before the listener
        // callback could run.
