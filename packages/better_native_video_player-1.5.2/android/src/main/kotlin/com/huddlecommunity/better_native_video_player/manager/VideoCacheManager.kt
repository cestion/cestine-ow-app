...
object VideoCacheManager {
    ...
    private const val CACHE_MIN_BYTES = 100L * 1024 * 1024
    ...
    /**
     * Calculates optimal cache size based on device RAM.
     *
     * @param context Application context for accessing ActivityManager
     * @return Cache size in bytes: 100MB (≤4GB RAM), 200MB (≤6GB), 300MB (>6GB)
     */
    fun calculateCacheSize(context: Context): Long {
        val activityManager = context.getSystemService(Context.ACTIVITY_SERVICE) as? ActivityManager
            ?: return CACHE_MIN_BYTES
        val memInfo = ActivityManager.MemoryInfo()
        activityManager.getMemoryInfo(memInfo)
        val totalRamBytes = memInfo.totalMem
        val totalRamGB = totalRamBytes / (1024L * 1024L * 1024L)
        return when {
            totalRamGB >= 8 -> CACHE_MAX_BYTES
            totalRamGB >= 6 -> 200L * 1024 * 1024
            else -> CACHE_MIN_BYTES
        }
    }

    /**
     * Returns the shared cache, creating it on first use. [maxBytes] from
     * Dart config is used as a floor; RAM-based sizing may increase it for
     * high-memory devices. The first caller wins (same applies-at-first-
     * creation semantics as androidBufferConfig).
     */
    @Synchronized
    fun getOrCreate(context: Context, maxBytes: Long): SimpleCache? {
        val effective = maxOf(maxBytes, calculateCacheSize(context))
        cache?.let { return it }
        if (creationFailed) return null
        return try {
            val created = SimpleCache(
                File(context.applicationContext.cacheDir, CACHE_DIR_NAME),
                LeastRecentlyUsedCacheEvictor(effective),
                StandaloneDatabaseProvider(context.applicationContext)
            )
            cache = created
            NpLog.d(TAG, "Disk cache created (max ${effective / (1024 * 1024)}MB, user=${maxBytes / (1024 * 1024)}MB, ram=${calculateCacheSize(context) / (1024 * 1024)}MB)")
            created
        } catch (e: Exception) {
     ...
     * the cache is unavailable. [maxBytes] from Dart config serves as a floor;
     * RAM-based sizing may increase it. The per-load upstream keeps its
     * headers, so cache misses fetch exactly what an uncached load would.
     */
    ...
    /**
     * Bytes currently held by the disk cache. Prefers [SimpleCache.cacheSpace]
     * when the cache is already open; otherwise walks the cache directory so
     * leftovers from a previous session are still reported.
     */
    ...
    /**
     * Drops every cached resource. Must go through [SimpleCache.removeResource]
     * when the cache is open — deleting files under a live SimpleCache can
     * crash the process.
     */
    fun clear(context: Context, callback: () -> Unit) {
        cancelAllPrecache()
        precacheExecutor.execute {
            try {
                doClear(context)
            } catch (e: Exception) {
                NpLog.e(TAG, "Disk cache clear failed: ${e.message}", e)
            }
    ...
    @Synchronized
    private fun doClear(context: Context) {
        val existing = cache
        if (existing != null) {
            for (key in existing.keys.toList()) {
                try {
                    existing.removeResource(key)
                } catch (e: Exception) {
                    NpLog.w(TAG, "Failed to remove cache key $key: ${e.message}")
                }
            }
            return
        }
        ...
        if (dir.exists() && !dir.deleteRecursively()) {
            NpLog.w(TAG, "Failed to delete unused cache dir ${dir.absolutePath}")
        }
    }
