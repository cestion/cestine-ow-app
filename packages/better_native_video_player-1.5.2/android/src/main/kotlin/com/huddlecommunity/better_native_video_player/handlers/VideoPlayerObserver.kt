...
class VideoPlayerObserver(
    ...
    private var lastFrameHeartbeatMs = 0L
    private val frameHeartbeatIntervalMs = 250L
    ...
    /**
     * Media3 invokes this after the first frame has reached the configured
     * output surface. Unlike position/isPlaying, this proves the video path
     * has produced a presentable frame.
     */
    override fun onRenderedFirstFrame() {
        if (isReleased) return
        val nowMs = SystemClock.elapsedRealtime()
        lastFrameHeartbeatMs = nowMs
        eventHandler.sendEvent(
            "firstFrame",
            mapOf(
                "renderedAtMs" to nowMs,
    ...
    /**
     * Throttled frame heartbeat from ExoPlayer's render pipeline. This callback
     * runs for decoded frames immediately before surface submission; sending
     * every frame would overload Flutter's EventChannel in a three-player feed.
     */
    override fun onVideoFrameAboutToBeRendered(
        ...
        if (isReleased) return
        val nowMs = SystemClock.elapsedRealtime()
        if (nowMs - lastFrameHeartbeatMs < frameHeartbeatIntervalMs) return
        lastFrameHeartbeatMs = nowMs
        handler.post {
